...
export class DefinitionAddComponent implements OnInit {
    ...
    deleteProxiedPort(
        proxiedPort: DefinitionAddFormProxiedPortFormElement,
    ): void {
    ...
    deleteEnvVariable(
        envVariable: DefinitionAddFormEnvVariableFormElement,
    ): void {
    ...
    deleteSummaryItem(
        summaryItem: DefinitionAddFormSummaryItemFormElement,
    ): void {
    ...
    deleteDownloadable(
        downloadable: DefinitionAddFormDownloadableFormElement,
    ): void {
    ...
    protected mapItem(): any {
        const mappedItem = {
            ...
            config: {
                ...
                proxiedPorts: this.item.config.proxiedPorts.map(
                    proxiedPort => ({
                        id: proxiedPort.id,
                ...
                composeFiles: [this.item.config.composeFile],
                actions: this.item.config.actions.map(action => ({
                    id: action.id,
                    name: action.name,
                    type: action.type,
                    afterBuildTasks: action.afterBuildTasks.map(
                        afterBuildTask => {
                            const mappedAfterBuildTask = _.cloneDeep(
                                afterBuildTask,
                            );
                            ...
                            if (
                                'executeHostCommand' === afterBuildTask.type ||
                                'executeServiceCommand' === afterBuildTask.type
                            ) {
                                // @ts-ignore
                                ...
                                for (const inheritedEnvVariable of afterBuildTask.inheritedEnvVariables) {
                                    if (
                                        /^ *$/.test(inheritedEnvVariable.alias)
                                    ) {
    ...
    protected mapYamlConfig(
        yamlConfig: any,
    ): DefinitionAddFormConfigFormElement {
        // TODO Check schema validity of Yaml config.

        const camelCaseYamlConfig = camelCaseKeys(jsYaml.safeLoad(yamlConfig), {
            deep: true,
        });
        ...
        mappedYamlConfig.composeFile = {
            ...
            envDirRelativePath:
                camelCaseYamlConfig.composeFiles[0].envDirRelativePath,
            composeFileRelativePaths:
                camelCaseYamlConfig.composeFiles[0].composeFileRelativePaths,
        };
    ...
    protected getProject(): void {
        ...
        this.route.params
            ...
            .subscribe((item: GetProjectQueryProjectFieldInterface) => {
                this.project = item;
                this.spinner.hide();
            });
    }
