...
export class AssetResolverFactory {
    ...
    ): (object: object, args: object) => Promise<AssetTypeInterface[]> {
        return async (
            object: object,
            args: object,
    ...
    ): (obj: any, args: any) => Promise<AssetTypeInterface> {
        return async (obj: any, args: any): Promise<AssetTypeInterface> => {
            const assets = await this.assetRepository.find(
                { id: idExtractor(obj, args) },
                0,
                1,
            );
            if (assets.length !== 1) {
    ...
    public getCreateItemResolver(): (
        _: any,
        createAssetInput: CreateAssetInputTypeInterface,
    ) => Promise<AssetTypeInterface> {
        return async (
            _: any,
            createAssetInput: CreateAssetInputTypeInterface,
