...
@Injectable()
export class ExecuteHostCmdCommandExecutorComponent
    ...
    execute(command: SimpleCommand): Promise<any> {
        ...
        return new Promise<any>((resolve, reject) => {
            ...
            for (const {
                name,
                value,
            } of typedCommand.customEnvVariables.toList()) {
            ...
            logger.info(
                `Arguments: ${JSON.stringify(typedCommand.command.slice(1))}`,
            );
            logger.info(
                `Guest working directory: ${typedCommand.absoluteGuestInstancePath}`,
            );
            logger.info(
