...
@Injectable()
export class CreateVolumeFromAssetCommandExecutorComponent
    ...
    async execute(command: SimpleCommand): Promise<any> {
        ...
        const asset = await this.assetHelper.findUploadedById(
            typedCommand.assetId,
        );
        ...
        return new Promise((resolve, reject) => {
            const spawned = spawn(
                ...
                [
                    ...
                    '--rm',
                    '-v',
                    `${absoluteUploadedAssetHostPath}:/source.tar.gz`,
                    '-v',
                    `${volumeName}:/target`,
                    'alpine',
                    'ash',
                    '-c',
                    'tar -zxvf /source.tar.gz -C /target/',
                ],
                { cwd: workingDirectory },
            );
            ...
            this.spawnHelper.handleSpawned(
                ...
                (exitCode: number) => {
                    logger.error(
                        `Failed to copy files from asset to volume.`,
                        {},
                    );
                    logger.error(`Exit code: ${exitCode}`, {});
                },
