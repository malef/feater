        ...
        return new Promise((resolve, reject) => {
            ...
            for (const absoluteGuestComposeFilePath of typedCommand.absoluteGuestComposeFilePaths) {
                dockerComposeArgs.push([
                    '--file',
                    path.relative(
                        typedCommand.absoluteGuestEnvDirPath,
                        absoluteGuestComposeFilePath,
                    ),
                ]);
            }
            ...
            runEnvVariables.add(
                ...
                `${environment.instantiation.composeHttpTimeout}`,
            );
            ...
            logger.info(
                `Binary: ${environment.instantiation.composeBinaryPath}`,
            );
            logger.info(`Arguments: ${JSON.stringify(dockerComposeArgs)}`);
            logger.info(
                `Guest working directory: ${typedCommand.absoluteGuestEnvDirPath}`,
            );
            logger.info(
