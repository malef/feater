...
export class VariablesPredictor {
    predictEnvVariables(
        ...
        const proxiedPortIds = _.filter(
            _.map(definitionConfig.proxiedPorts, 'id'),
        );
        envVariables.push(
            proxiedPortIds.map(
                (proxiedPortId: string): PredictedEnvVariableInterface => {
                    return {
                        ...
                        pattern: environment.instantiation.proxyDomainPattern.replace(
                            '{port_id}',
            ...
            volumeIds.map(
                (volumeId: string): PredictedEnvVariableInterface => {
                    return {
                        ...
                        pattern: `${
                            environment.instantiation.containerNamePrefix
                        }{instance_hash}_${volumeId.toLowerCase()}`,
                    };
                },
            ),
        );
